        ...
        {Object.entries(PLAN_DISPLAY).map(([planId, plan]) => (
          <div
            ...
            className={cn(
              ...
              planId === 'profissional'
                ? 'border-brand-teal ring-4 ring-brand-teal/10 z-10'
                : 'border-gray-100 dark:border-white/10 opacity-95 hover:opacity-100 shadow-sm'
            )}
          >
            {/* Background Accent sutil */}
            {planId === 'profissional' && (
              <div className="absolute -right-12 -top-12 w-40 h-40 bg-brand-teal/10 rounded-full blur-3xl" />
            )}

            {/* Badge destaque */}
            {planId === 'profissional' && (
              <span className="absolute -top-3.5 inset-x-0 mx-auto w-fit text-xs font-bold uppercase tracking-widest bg-gradient-to-r from-brand-teal to-brand-blue text-white px-5 py-1.5 rounded-full shadow-lg">
                Mais Recomendado
              </span>
            )}
            ...
            <div className="mb-8">
              <span className="text-4xl font-extrabold text-brand-blue dark:text-brand-teal tracking-tight">{plan.price}</span>
            </div>
            ...
            <ul className="space-y-4 flex-1 mb-10">
              {plan.features.map((feature) => (
                <li key={feature} className="flex items-start gap-4 text-sm text-gray-600 dark:text-gray-300 font-medium leading-tight">
                  <div className="bg-brand-teal/10 dark:bg-brand-teal/20 p-1 rounded-full shrink-0 border border-brand-teal/20">
                    <Check size={14} className="text-brand-teal" strokeWidth={3} />
                  </div>
                  <span>{feature}</span>
                </li>
              ))}
            ...
            <a
              ...
              className={cn(
                ...
                planId === 'profissional'
                  ? 'bg-brand-teal text-white hover:bg-brand-blue shadow-lg hover:shadow-xl hover:-translate-y-1'
